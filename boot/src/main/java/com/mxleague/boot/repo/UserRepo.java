...
public interface UserRepo extends CrudRepository<User, String> {
	...
	@Query("SELECT u FROM User u WHERE LOWER(u.id_user) = LOWER(?1) AND LOWER(u.role.id_role) = LOWER(?2)")
	User findById_UserAndId_RoleCaseInsensitive(String id_user, String id_role);

	@Query("SELECT u FROM User u WHERE LOWER(u.id_user) = LOWER(?1) AND LOWER(u.player.id_player) = LOWER(?2)")
	User findById_UserAndId_PlayerCaseInsensitive(String id_user, String id_player);
	
	@Query("SELECT u FROM User u WHERE LOWER(u.id_user) = LOWER(?1) AND LOWER(u.board.id_employee) = LOWER(?2)")
